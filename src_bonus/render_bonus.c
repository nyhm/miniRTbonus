...
/*   Updated: 2025/06/06 23:15:07 by hnagashi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int is_cylinder_bottom_checkerboard(t_vec3 point, t_cylinder *cy, double scale)
{
    ...
    t_vec3 v;
    v = vec_cross(axis, u);
    double x_2d;x_2d = vec_dot(diff, u) * scale;
    double y_2d;y_2d = vec_dot(diff, v) * scale;

    int checker_x;checker_x = (int)floor(x_2d);
    int checker_y;checker_y = (int)floor(y_2d);
...
{
    t_vec3 axis;axis = cy->direction;
    t_vec3 cp ;cp = vec_sub(point, cy->center);

    double height;height = vec_dot(cp, axis);

    if (height < 0.001) 
        return (is_cylinder_bottom_checkerboard(point, cy, scale_bottom));
    else {
        double scaled_height ;scaled_height = height * scale_height;

        t_vec3 radial;radial = vec_sub(cp, vec_mul(axis, height));
        double angle; angle = atan2(radial.z, radial.x) * scale_angle;

        int checker_x;checker_x = (int)floor(scaled_height);
        int checker_y;checker_y = (int)floor(angle);
...
int is_checkerboard_z(t_vec3 point, double scale)
{
    ...
    int y ;y = (int)floor(point.y * scale);
    int x ;x = (int)floor(point.x * scale);
    return ((y + x) % 2 == 0);
}
...
int is_checkerboard_x(t_vec3 point, double scale)
{
    ...
    int y;y = (int)floor(point.y * scale);
    int z;z = (int)floor(point.z * scale);
    return ((y + z) % 2 == 0);
}
...
{
    t_vec3 p_centered;p_centered = vec_sub(point, center);

    double theta;
    theta = atan2(p_centered.z, p_centered.x);
    if (theta < 0)
        theta += 2 * M_PI;
    double phi;phi = acos(p_centered.y / radius);

    int check_u;check_u = (int)(theta * scale);
    int check_v;check_v = (int)(phi * scale);
...
void find_closest_plane(t_ray ray, t_data *data, t_color *color, t_hit_record *record)
{
    ...
    {
        double t_hit;t_hit = 0.0;
        if (hit_plane(&data->scene->planes[i], ray, &t_hit)
            && t_hit < record->t && t_hit > 0.001)
        {
            t_plane *pl;pl = &data->scene->planes[i];
            record->t = t_hit;
            record->plane_index = i;
            record->sphere_index = -1;
            record->cylinder_index = -1;

            t_vec3 hit_point;hit_point = vec_add(ray.origin, vec_mul(ray.direction, t_hit));
            t_color base;base = pl->color;
            t_color alt ;alt = opposite_color(base);
    ...
    while ( i < data->scene->sphere_count)
    {
            ...
            && t_hit < record->t && t_hit > 0.001)
        {
            ...
            *color = data->scene->spheres[i].color;
            ...
            record->cylinder_index = -1;
            t_vec3 hit_point;hit_point = vec_add(ray.origin, vec_mul(ray.direction, t_hit));
            t_vec3 normal;normal = vec_normalize(vec_sub(hit_point, data->scene->spheres[i].center));
    ...
    while( i < data->scene->cylinder_count)
    {
            ...
            && t_hit < record->t && t_hit > 0.001)
        {
            ...
            *color = data->scene->cylinders[i].color;
            ...
            record->plane_index = -1;
            t_vec3 hit_point;hit_point = vec_add(ray.origin, vec_mul(ray.direction, t_hit));
            t_cylinder *cy;cy = &data->scene->cylinders[i];
            t_vec3 cp;cp = vec_sub(hit_point, cy->center);
            double proj;proj = vec_dot(cp, cy->direction);
            t_vec3 normal;normal = vec_normalize(vec_sub(cp, vec_mul(cy->direction, proj)));
...
{
    t_vec3 light_dir;light_dir = vec_normalize(vec_sub(light.pos, point));
    double light_distance;light_distance = vec_len2(vec_sub(light.pos, point));
    t_ray shadow_ray;shadow_ray = (t_ray){ .origin = vec_add(point, vec_mul(light_dir, 1e-4)), 
                         .direction = light_dir };
    int i;i = 0;
    while( i < data->scene->plane_count)
    {
...
{
    t_vec3 light_dir;light_dir = vec_normalize(vec_sub(light.pos, hit_point));
    t_vec3 view_dir;view_dir = vec_normalize(vec_sub(data->scene->camera.pos, hit_point));

    double ambient_strength;ambient_strength = data->scene->ambient.brightness;
    double diffuse_strength;diffuse_strength = 0.0;
    double specular_strength;specular_strength = 0.5;
    double shininess;shininess = 64.0;

    double r;r = base.r * ambient_strength * light.color.r / 255.0;
    double g;g = base.g * ambient_strength * light.color.g / 255.0;
    double b;b = base.b * ambient_strength * light.color.b / 255.0;

    if (!is_in_shadow(hit_point, light, data))
    {
        diffuse_strength = fmax(vec_dot(normal, light_dir), 0.0);

        t_vec3 reflect_dir;reflect_dir = vec_reflect(vec_mul(light_dir, -1), normal);
        double spec;spec = pow(fmax(vec_dot(view_dir, reflect_dir), 0.0), shininess);
...
{
    t_color final;final = (t_color){0, 0, 0};
    t_color base;base = (t_color){100, 100, 100}; 
    t_hit_record record;record = (t_hit_record){
        .t = INFINITY,
        .sphere_index = -1,
        .plane_index = -1,
        .cylinder_index = -1,
    ...
    if (record.t < INFINITY)
    {
        ...
        {
            t_light light;light = data->scene->lights[i];
            t_color light_result;light_result = apply_lighting(base, record.hit_point, record.normal, light, data);
...
{
    double u;u = (x + (sx + 0.5) / SAMPLES) / WIDTH;
    double v;v = (y + (sy + 0.5) / SAMPLES) / HEIGHT;

    double scale;scale = tan(data->scene->camera.fov * 0.5 * M_PI / 180.0);
    double px;px = (2 * u - 1) * data->scene->camera.aspect_ratio * scale;
    double py;py = (1 - 2 * v) * scale;

    t_vec3 dir;dir = vec_normalize(vec_add(
        vec_add(
...
void render_pixel(t_data *data,int x,int y)
{
    ...
    while ( sy < SAMPLES) {
        ...
        while ( sx < SAMPLES)
        {
            t_ray ray;ray = generate_camera_ray(data, x, y, sx, sy);
            t_color sample;sample = trace_ray(ray, data);
            r += sample.r;
            g += sample.g;
            b += sample.b;
            sx++;
    ...
    t_color final_color;final_color = (t_color){
        r / (SAMPLES * SAMPLES),
        g / (SAMPLES * SAMPLES),
        b / (SAMPLES * SAMPLES)
    };
...
{
    t_thread_data *td;td = (t_thread_data *)arg;
    int x;
    int y;
    y = td->start_y; 
    while(y < td->end_y) {
...
void render(t_data *data)
{
    ...
    double elapsed_time;elapsed_time = (end.tv_sec - start.tv_sec) +
